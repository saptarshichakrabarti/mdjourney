...
Adapted for gateway-based architecture.
"""
...
backend_dir = Path(__file__).parent / "mdjourney-backend"
  ...
  mdjourney version                  # Show version information
    ...
    try:
        if args.command == "setup":
            ...
            result = subprocess.run(test_args, cwd=backend_dir)
            ...
            result = subprocess.run(["python", str(backend_dir / "scripts" / "lint.py")], cwd=backend_dir)
            return result.returncode
            ...
            result = subprocess.run(["black", "."], cwd=backend_dir)
            ...
            result = subprocess.run(["isort", "."], cwd=backend_dir)
            if result.returncode != 0:
            ...
            logger.info("MDJourney 1.0.0")
            logger.info("FAIR-compliant research data metadata automation system")
            ...
            logger.info("  - Gateway: Session-based backend management")

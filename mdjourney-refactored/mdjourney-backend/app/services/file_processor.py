...
class FileProcessor:
    ...
    def __init__(self, scanner: Optional[IFileScanner] = None, schema_manager=None, vc_manager=None) -> None:
        """
        Initialize the file processor.

        Args:
            scanner: File scanner implementation to use. If None, will use DirmetaScanner.
            schema_manager: Schema manager instance (optional, will be loaded if not provided)
            vc_manager: Version control manager instance (optional, will be loaded if not provided)
        """
        ...
        if vc_manager is None:
            try:
                from .version_control import get_vc_manager
    ...
    def process_new_file(self, file_path: str, dataset_path: str) -> bool:
        ...
        try:
            ...
            if success and self.vc_manager:
    ...
    def _update_dataset_structural_file(
        ...
        try:
            ...
            # Validate updated data against schema if schema_manager is available
            if self.schema_manager:
                try:
                    struct_schema = self.schema_manager.get_dataset_struct_schema()
                    if not self.schema_manager.validate_json(struct_data, struct_schema):
                        print(
...
def process_file_with_dirmeta(file_path: str, dataset_path: str) -> bool:
    """Process a file with dirmeta (convenience function)."""
    processor = FileProcessor()
    return processor.process_new_file(file_path, dataset_path)

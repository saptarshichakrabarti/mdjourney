...
Adapted for refactored architecture.
"""
...
def build_frontend():
    ...
    frontend_dir = project_root / "mdjourney-webapp"
...
def clean_build_artifacts():
    ...
    for pattern in dirs_to_clean:
        if "*" in pattern:
            ...
            run_command(f"rm -rf {pattern}", cwd=project_root, check=False)
        else:
...
def check_package_health():
    ...
    if manifest_path.exists():
        try:
            run_command("check-manifest --ignore mdjourney-webapp/node_modules,mdjourney-webapp/dist", cwd=project_root)
...
def build_package():
    ...
    # Check the package
    run_command("twine check dist/*", cwd=project_root)

...
def validate_configuration(config_path: str) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        schemas_config = config_manager.get_setting('schemas', {})
        if schemas_config:
            custom_path = schemas_config.get('custom_path')
            if custom_path and custom_path.strip():
                custom_path_obj = Path(custom_path)
                if not custom_path_obj.exists():
                    errors.append(f"Custom schema path does not exist: {custom_path}")
...
def check_environment_variables() -> List[str]:
    ...
    for var in env_vars_to_check:
        if var not in os.environ:
            warnings.append(f"Environment variable {var} is not set (optional)")
...
def print_configuration_summary(config_manager: ConfigManager):
    ...
    schemas_config = config_manager.get_setting('schemas', {})
    custom_path = schemas_config.get('custom_path')
    print(f"Custom Schema Path: {custom_path if custom_path else 'Not set'}")
...
def main():
    ...
    is_valid, errors = validate_configuration(str(config_path))
